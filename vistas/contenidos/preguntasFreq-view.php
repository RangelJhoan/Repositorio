<!DOCTYPE html>
<html lang="en">
<head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Repositorio Institucional</title>
</head>
<body class="bodyFQ">
<h2 class="fq-main-Title">Preguntas frecuentes</h2>
    <section class="wrapperFAQ">
        <!-- Pregunta #1 -->
        <div class="parentTabFAQ">
            <input type="radio" name="tab" id="tab-1" class="inputFAQ">
            <label for="tab-1" class="labelFAQ">
                <span class="questionFAQ">¿Qué es un repositorio institucional?</span>
                <div class="iconContainer">
                    <div class="iconFAQ">
                        <i class="uil uil-angle-down"></i>
                    </div>
                </div>
            </label>
            <div class="contentFAQ">
                <p class="answerFAQ">Un repositorio institucional es un espacio en donde se reúne, preserva y divulgan archivos en formato digital con información obtenida a través de desarrollos científicos o académicos de una institución. Se caracterizan porque las Instituciones de Educación Superior (IES) son las encargadas de generar y suministrar los contenidos que se encuentren disponibles.</p>
            </div>
        </div>
        <!-- Pregunta #2 -->
        <div class="parentTabFAQ">
            <input type="radio" name="tab" id="tab-2" class="inputFAQ">
            <label for="tab-2" class="labelFAQ">
                <span class="questionFAQ">¿Para quiénes va dirigido el repositorio institucional?</span>
                <div class="iconContainer">
                    <div class="iconFAQ">
                        <i class="uil uil-angle-down"></i>
                    </div>
                </div>
            </label>
            <div class="contentFAQ">
                <p class="answerFAQ">El repositorio va dirigido principalmente a usuarios con énfasis en contenidos de Ingeniería de Sistemas. Sin embargo, existe la posibilidad de poder publicar y encontrar recursos enfocados en otros programas académicos.</p>
            </div>
        </div>
        <!-- Pregunta #3 -->
        <div class="parentTabFAQ">
            <input type="radio" name="tab" id="tab-3" class="inputFAQ">
            <label for="tab-3" class="labelFAQ">
                <span class="questionFAQ">¿Qué tipo de archivos se pueden encontrar en el repositorio institucional?</span>
                <div class="iconContainer">
                    <div class="iconFAQ">
                        <i class="uil uil-angle-down"></i>
                    </div>
                </div>
            </label>
            <div class="contentFAQ">
                <p class="answerFAQ">En este repositorio institucional se pueden encontrar libros, vídeos, imágenes, audios, diapositivas, enlaces, entre otros. 
                </p>
            </div>
        </div>
        <!-- Pregunta #4 -->
        <div class="parentTabFAQ">
            <input type="radio" name="tab" id="tab-4" class="inputFAQ">
            <label for="tab-4" class="labelFAQ">                
                <span class="questionFAQ">¿Quiénes pueden tener acceso a los contenidos publicados en el repositorio institucional?</span>
                <div class="iconContainer">
                    <div class="iconFAQ">
                        <i class="uil uil-angle-down"></i>
                    </div>
                </div>
            </label>
            <div class="contentFAQ">
                <p class="answerFAQ">Los contenidos que se encuentren disponibles en el repositorio pueden ser accedidos por todos los usuarios que deseen, tengan o no tengan una cuenta asociada. Esto es así, ya que se tiene la premisa del acceso abierto libre para todos, de forma que se tenga una visibilidad total y una completa difusión de los documentos publicados.</p>
            </div>
        </div>
        <!-- Pregunta #5 -->
        <div class="parentTabFAQ">
            <input type="radio" name="tab" id="tab-5" class="inputFAQ">
            <label for="tab-5" class="labelFAQ">
                <span class="questionFAQ">¿Cómo puedo publicar un recurso académico?</span>
                <div class="iconContainer">
                    <div class="iconFAQ">
                        <i class="uil uil-angle-down"></i>
                    </div>
                </div>
            </label>
            <div class="contentFAQ">
                <p class="answerFAQ">Las únicas personas autorizadas para poder publicar recursos en el repositorio institucional son los docentes y administradores. Si usted es un usuario particular, por favor póngase en contacto al correo <a href = "mailto: admin.repositorioinstitucional@gmail.com">admin.repositorioinstitucional@gmail.com</a></p>
            </div>
        </div>
        <!-- Pregunta #6 -->
        <div class="parentTabFAQ">
            <input type="radio" name="tab" id="tab-6" class="inputFAQ">
            <label for="tab-6" class="labelFAQ">
                <span class="questionFAQ">¿Cómo puedo aportar al repositorio institucional?</span>
                <div class="iconContainer">
                    <div class="iconFAQ">
                        <i class="uil uil-angle-down"></i>
                    </div>
                </div>
            </label>
            <div class="contentFAQ">
                <p class="answerFAQ">Con el ánimo de poder contar con un repositorio con contenidos de valor para la comunidad, se le permite al usuario aportar por medio de calificaciones a los recursos, marcando si estos han sido útiles o no. De esta manera, en el repositorio se puede realizar depuración, dejando así únicamente los contenidos que mayor aporte han tenido para los usuarios.</p>
            </div>
        </div>
    </section>
</body>
</html>
